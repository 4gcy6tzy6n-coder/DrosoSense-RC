...
# be used. Skip-existing is the runner default.
#
# UNIT IDENTITY (DATA-59 D3-gate fix, Mika ruling 14:25 + 14:35): every size
# gets its OWN experiment label e9_size_d3_n<N> (same scheme as the D2
# smoke). §17 prior-touches are scoped by (dataset, experiment), so the
# five size arms live in five disjoint unit spaces and no cross-size skip
# can ever happen. Labels:
#
...
#
# DATA-59: committed, NOT launched. D3 waits for Mika's go-ahead AFTER the
# D2 smoke per-size table passes the D3-gate criteria.
set -euo pipefail
...
echo "E9/D3 size sweep (DATA-59, per-size labels, seed-half sharding); logs: $LOGDIR/e9_size_d3_${STAMP}_*.log"
cd "$REPO"
run_seed_half() {
  local tag="$1"; shift
  for seed in "$@"; do
    for size in 250 500 1000 2000 4000; do
      env DROSOSENSE_DATA=$DATA /root/miniconda3/bin/python -u \
          ...
          --experiment "e9_size_d3_n${size}" --seeds "$seed" --window-lengths 16 \
...
echo "records: find results/raw/e9_size_d3_n<size> -name '*.json' (5 sizes x 7 families x 62 folds x 10 seeds x 2 tasks)"
