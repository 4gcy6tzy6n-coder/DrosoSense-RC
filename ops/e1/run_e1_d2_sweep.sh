#!/usr/bin/env bash
# E1 baseline sweep — D2 (beef-uncontrolled), pre-staged by Mika 2026-09-21 (v2).
...
echo "markers OK; logs: $LOGDIR/e1_d2_sweep_${STAMP}_*.log"
cd "$REPO"
common=(scripts/run_baselines.py --dataset d2_beef_uncontrolled --experiment e1_main_d2 \
...
run() { local tag="$1"; shift; env DROSOSENSE_DATA=$DATA /root/miniconda3/bin/python -u "$@" \
        > "$LOGDIR/e1_d2_sweep_${STAMP}_${tag}.log" 2>&1 & }
...
wait
echo "D2 sweep finished; logs: $LOGDIR/e1_d2_sweep_${STAMP}_*.log"
