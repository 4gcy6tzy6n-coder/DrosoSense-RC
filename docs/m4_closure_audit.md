...
- **R0_vs_R3 是唯一 Holm 后 <0.05 的对比（0.0436→0.1742 后仍 <0.05）**：
  效应为 R3（随机稀疏）以 0.00118 macro_f1 的幅度**显著优于 R0**。
  注意 D3 上 n_clusters_nonzero=42、最小可达 p=4.5e-13，显著性**可达**
  （不像 D2 的 0.0625 floor）；这是真实的可达显著结果，方向对 R0 不利。
...
（R2）及匹配规模 ESN（R4）在 D3 上**统计等价**；唯一显著差异是
随机稀疏 R3 小幅优于 R0（Δ=−0.00118 macro_f1，Holm p=0.174，**未达
显著**，只能作描述报告）。**不得**保留"生物接线有意义"的主张；
...
|---|---|---|---|---|
| H1：R0 > R2（主假设，macro_f1） | E2-D3 | Δ=−0.000101, p=0.108, rank-biserial=0.042 | [−0.000687, +0.000489] | **不支持**（且 TOST 等价 ⇒ 报等价而非"无差异"） |
| H2：R0 > R4 | E2-D3 | Δ=−0.000434, p_Holm=1.0 | [−0.001382, +0.000422] | **不支持**（等价成立 ⇒ 描述为等价） |
| H3：R0 > R3 | E2-D3 | Δ=−0.001178, p_Holm=0.1742 | [−0.002261, −0.000197] | **不支持**（方向相反、未达 Holm 显著；只能作描述 + N3 型负结果叙事） |
| H4：R0 > GRU | — | unpairable（model-zoo 绑定形状缺口，§4.1） | — | **不可评**（[需补充]：补 `gru` 侧 E2 批次或修订 model-zoo 绑定走协议版本） |
| 回归侧主对比 R0 vs R2 | E2-D3 | Δmae=−0.000215, p=0.7035 | [−0.000793, +0.000367] | **不支持**（等价） |
| D2 任意 topology 显著性 | E2-D2 | 全部 floor-limited（0.0625 > 0.05） | 见 §4.2 | **不可达**（underpowered，按协议报告） |
| 扰动/低数据鲁棒性（E3–E5） | — | 批次不存在 | — | PENDING |
| 效率（E7 / params 半边） | E2 raw 记录 | R0=1,004 trainable vs GRU=4,452 max | — | 数值成立（§5.1 A2 半边）；效率主张的完整对照（时延/内存/MACs）**未做**，不得写成效率结论 |
...
3. **n0_raw 口径裁示**（§3，HIGH）：Mika 需裁示正式对照口径
   （补 n0_raw 新批次 vs 接受 n1_pre_l1 记录）；
...
| 级 | 项 | 证据 | 处置 |
|---|---|---|---|
| HIGH | 正式 E2 批次用 `n1_pre_l1`，与 11:40Z"必须 n0_raw"裁定不一致（后补"另记理由"授权在前） | §3；raw 记录 `normalization` 字段 | 已逐字登记；待 Mika 裁示；不重跑、不改协议 |
